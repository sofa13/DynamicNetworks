...
public class Topology extends _Properties implements ClockListener{
    ...
    DynamicEngine dynamicEngine=null;
    ...
    public Topology(int width, int height, boolean toBeStarted){
        ...
        // add two clocks
        clock.add(new Clock(this,0));
        clock.add(new Clock(this,1));
        if (! toBeStarted)
        	for (Clock aclock : clock)
        		aclock.pause();
    ...
    */
    ...
    /**
     * Gets a reference on the dynamic engine of this topology.
     */
    public DynamicEngine getDynamicEngine() {
        return dynamicEngine;
    }
    /**
     * Sets the dynamic engine of this topology.
     */
    public void setDynamicEngine(DynamicEngine dynamicEngine, DynamicEngine.Type type) {
        this.dynamicEngine = dynamicEngine;
        dynamicEngine.setTopology(this, type);
    }
     ...
     * @return The duration
     */
    public int getClockSpeed(int index){
        return clock.get(index).getTimeUnit();
    }
     ...
     */
    public void setClockSpeed(int period, int index){
    	clock.get(index).setTimeUnit(period);
    }
    ...
    public int getTime(int index){
        return clock.get(index).currentTime();
    }
    /**
    ...
    public boolean isRunning(int index){
        return clock.get(index).isRunning();
    }
    ...
    public void addClockListener(ClockListener listener, int period){
    	for(Clock aclock: clock)
    		aclock.addClockListener(listener, period);
    }
    /**
     * Registers the specified listener to the events of the topology clock.
     * @param listener The listener to register.
    ...
    public void addClockListener(ClockListener listener){
    	for(Clock aclock: clock)
    		aclock.addClockListener(listener);
    }
    /**
    ...
    public void removeClockListener(ClockListener listener){
    	for(Clock aclock: clock)
    		aclock.removeClockListener(listener);
    }
